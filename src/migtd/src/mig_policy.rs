...
pub mod v2 {
    ...
    use policy::{
        v2::{
            collateral::{
                self, get_collateral_with_fmspc, get_fmspc_from_quote,
                get_tcb_evaluation_number_from_collateral, verify_collateral_integrity,
            },
            get_engine_svn_from_map, verify_engine_integrity, verify_policy_integrity, MigPolicy,
            PolicyEvaluationInfo, TcbStatus,
        },
        verify_event_log, PolicyError, Report,
    };
    ...
    lazy_static! {
        ...
        pub static ref INIT_TCB_INFO: Vec<PolicyEvaluationInfo> = Vec::new();
    }

    pub fn init_policy_v2() -> Result<(), PolicyError> {
        let td_report_local = tdx_tdcall::tdreport::tdcall_report(&[0u8; 64])
    ...
    pub fn authenticate_migration_source(
        ...
        let local_tcb_evaluation_info = get_local_tcb_evaluation_info()?;
        mig_policy_obj.evaluate_policy_common(&tee_tcb_info_src, &local_tcb_evaluation_info)?;
        mig_policy_obj.evaluate_policy_forward(&tee_tcb_info_src, &local_tcb_evaluation_info)?;

        // Verify the destination's policy
        mig_policy_obj.evaluate_against_policy(&mig_policy_obj_src, &local_tcb_evaluation_info)
    }
}
...
mod dummy {
    ...
    pub fn tdcall_servtd_ext() -> Result<ServtdExt, TdCallError> {
        Ok(ServtdExt {
