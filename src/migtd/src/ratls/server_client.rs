...
use crate::{
    ...
    migration::{data::MigrationInformation, with_init_migtd_config},
};
use verify::*;
    ...
    with_init_migtd_config(&mig_info.mig_info, |init| {
        build_x509_common(quote, pub_key, |builder| {
            let cert = builder
                ...
                .add_extension(Extension::new(
                    EXTNID_MIGTD_COLLATERAL_INIT,
                    Some(false),
                    Some(&init.collaterals),
                )?)?
                .add_extension(Extension::new(
...
fn gen_cert_server(signing_key: &EcdsaPk, pub_key: &[u8], quote: &[u8]) -> Result<Vec<u8>> {
    let svn_map = get_engine().ok_or(RatlsError::Config)?;
    build_x509_common(quote, pub_key, |builder| {
        let cert = builder
            .add_extension(Extension::new(
                EXTNID_MIGTD_SVN_MAP,
                Some(false),
                Some(svn_map),
            )?)?
            .build();
