...
use crate::config::{get_collaterals, get_policy};
use crate::driver::ticks::TimeoutError;
use crate::event_log::get_event_log;
use crate::ratls::{
    RatlsError, EXTNID_MIGTD_COLLATERAL_INIT, EXTNID_MIGTD_EVENTLOG_INIT, EXTNID_MIGTD_POLICY_INIT,
...
fn parse_and_store_init_migtd_config(
...
fn parse_migtd_cert(cert: &[u8]) -> Option<InitMigtdConfig> {
    ...
    let mut collaterals = None;
    let mut td_report = None;
    let mut event_log = None;
    let mut mig_policy = None;

    for extn in extensions.get() {
        if extn.extn_id == EXTNID_MIGTD_COLLATERAL_INIT {
            collaterals = extn.extn_value.map(|v| v.as_bytes().to_vec());
        } else if extn.extn_id == EXTNID_MIGTD_TDREPORT_INIT {
...
pub fn put_current_migtd_config(
    ...
    let event_log = get_event_log()
        ...
        .to_vec();
    let collaterals = get_collaterals()
        .ok_or(MigrationResult::InvalidParameter)?
        .to_vec();
