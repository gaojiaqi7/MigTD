...
impl<'a> PolicyData<'a> {
    ...
    pub fn validate(&self) -> bool {
        ...
        self.policy
            .as_ref()
            .is_none_or(|common| Self::validate_policy_block(&common, PolicyBlockType::Common))
            && self.forward_policy.as_ref().is_none_or(|forward| {
                Self::validate_policy_block(&forward, PolicyBlockType::Forward)
            })
            && self.backward_policy.as_ref().is_none_or(|backward| {
                Self::validate_policy_block(&backward, PolicyBlockType::Backward)
            })
    }
        ...
        for policy_type in block {
            match policy_type {
                PolicyTypes::Global(global) => {
                    if !global.validate(block_type) {
                        return false;
                    }
                }
                PolicyTypes::Servtd(migtd) => {
                    if !migtd.validate(PolicyBlockType::Common) {
                        return false;
                    }
                }
            }
...
impl GlobalPolicy {
    fn validate(&self, block_type: PolicyBlockType) -> bool {
        if let Some(tcb_policy) = &self.tcb {
            if !tcb_policy.validate(block_type) {
                return false;
            }
        }

        if let Some(platform_policy) = &self.platform {
            if !platform_policy.validate(block_type) {
                return false;
            }
        }
...
impl TcbPolicy {
    fn validate(&self, block_type: PolicyBlockType) -> bool {
        if let Some(property) = &self.tcb_evaluation_data_number {
            if !property.validate(block_type) {
                return false;
            }
        }

        if let Some(tcb_status_policy) = &self.tcb_status_accepted {
            if !tcb_status_policy.validate(block_type) {
                return false;
            }
        }

        if let Some(tcb_date_policy) = &self.tcb_date {
            if !tcb_date_policy.validate(block_type) {
                return false;
            }
        }
